...
# die "Usage: $0 <VEP vcf>" unless (@ARGV == 1);
	...
	if (/^##INFO=\<ID=([^=,]+),/) {
		...
		if ($info eq "CSQ" || $info eq "ANN") {
			if (/Description=\"[^"=]+Format:\s+(.+)\"\>$/) {
				...
				if ($PICK_i eq "") {
					for (my $i = 0; $i < @csq_fields; $i ++) {
						$PICK_i = $i if ($csq_fields[$i] eq "CANONICAL");
					}
				}
			}
		} else {
		...
		if (/;?(?:CSQ|ANN)=([^;\s]+);?/) { # there could be equal signs "=" in annotations for synonymous variants, so don't use pattern /;?CSQ=([^=;\s]+);?/
			my $picked = 0;
			...
			for (my $i = 0; $i < @transcripts; $i ++) {
				...
				if ($csq_transcript[$PICK_i] eq "1" || $csq_transcript[$PICK_i] eq "YES") {
			...
			if ($picked == 0) { # If no transcript with PICK==1 or CANONICAL==YES, then choose the first transcript to output
				my @csq_transcript = split(/\|/, $transcripts[0], -1);
				for (my $i = 0; $i < @csq_transcript; $i ++) {
					$output .= ($csq_transcript[$i] eq "" ? "\t-" : "\t$csq_transcript[$i]");
				}
			}
